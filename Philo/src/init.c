...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/02 13:05:48 by fcosta-f          #+#    #+#             */
/*   Updated: 2024/03/02 13:19:44 by fcosta-f         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_all(t_all *program, int argc, char **argv)
{
	...
	if (argc > 5)
		program->total_meals = ft_atoi(argv[5]);
	else
		program->total_meals = -1;
	// printf("ha obtenido el 5 argumento\n");
	program->dead = 0;
	program->finish = 0;
	program->init_time = ft_get_time();
	// printf("init time es %d\n", program->init_time);
	pthread_mutex_init(&program->lock, NULL);
	pthread_mutex_init(&program->monitorize, NULL);
	pthread_mutex_init(&program->mutex_time_die, NULL);
	if (arrays_alloc(program))
		return (1);
	init_philos(program);
	init_forks(program);
	return (0);
